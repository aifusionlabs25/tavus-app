...
export async function POST(request: Request) {
    try {
        ...
        // FALLBACK: v18.7 Legacy Logic (Existing Code)
...
async function handleNativeMorgan(body: any) {
    ...
    // 2. Gemini Analysis (Hybrid: Objectives + Transcript)
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
    ...
    const analysisPrompt = `
      ...
      TASK:
      Generate a JSON object for the sales team. 
      - Use the STRUCTURED DATA as your primary source. 
      - Use the TRANSCRIPT to fill in gaps or add context.
      ...
      REQUIRED JSON OUTPUT:
      {
        ...
        "sales_strategy": "String (3-step plan based on pain points)"
      }
    `;
    ...
    try {
        ...
        // Fallback to raw structured data if Gemini fails
    ...
    const leadData: LeadData = {
        ...
        pain_points: extracted.pain_points || (structuredData.top_pain_points ? structuredData.top_pain_points : []),
    ...
    // 4. Save to Sheets & Create Draft (Reuse existing logic)
    // Note: We are duplicating the save logic here to keep the handler self-contained for now.
    // In a future refactor, we should extract 'saveToSheets' and 'createDraft' into shared functions.
    ...
    try {
        const values = [
            [
                ...
                leadData.budget_range,
                leadData.timeline,
                Array.isArray(leadData.pain_points) ? leadData.pain_points.join(', ') : leadData.pain_points,
                leadData.buying_committee.join(', '),
                leadData.vertical,
                leadData.teamSize,
                leadData.geography,
